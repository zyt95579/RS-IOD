...
model = dict(
    ...
    bbox_head=dict(
        type='GroundingDINOHead_inc_distn',
        setting='full_text',
...
test_pipeline = [
    ...
    dict(
        type='PackDetInputs',
        meta_keys=('img_id', 'img_path', 'ori_shape', 'img_shape',
                   'scale_factor', 'text', 'ori_text','custom_entities'))
]
...
default_hooks = dict(
    #checkpoint=dict(max_keep_ckpts=3, save_best='auto'),
    checkpoint=dict(save_optimizer=False, save_param_scheduler=False),
